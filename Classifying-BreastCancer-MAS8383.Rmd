...
```{r bss, echo=FALSE}
bss_fit <- regsubsets(Class ~.,data = dat[,2:11], method = "exhaustive",nvmax = 9)
...
## We use 5-sample cross validation
fold_index <- sample(5, nrow(dat), replace = TRUE)
...
best_model <- glm(Class~Cl.thickness + Cell.size + Bare.nuclei + Bl.cromatin + Normal.nucleoli, data = dat[,2:11])
...
lasso_fit = glmnet(dat[,2:10], dat$Class, alpha=1, standardize = FALSE, lambda = grid)
```
...
for (i in 1:9){
  ...
  for (j in 1:num_of_size_i){
    ...
    lda_fit = linDA(variables = as.matrix(dat[,all_subsets[i][[1]][j][[1]]]), group = dat$Class)
